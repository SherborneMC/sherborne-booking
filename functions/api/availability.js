...
export async function onRequestGet({request,env}){
  ...
  const scope=url.searchParams.get('scope')==='currentWeek'?'currentWeek':'full';
  try{
    const result=await buildAvailability(env,scope);
    ...
    return json({live:result.live,scope,pulledAt:result.pulledAt,slots:result.cells,error:result.live?undefined:'Diary connection unavailable'},status);
  }catch(e){
    ...
    return json({live:false,scope,pulledAt:new Date().toISOString(),slots:cells,error:'Diary connection unavailable'},503);
  }
}
